...
namespace CMF
{
    ...
    public class AdvancedWalkerController : Controller
    {
        ...
        ControllerState DetermineControllerState()
        {
            ...
            //Check if controller is colliding with a ladder
            bool _isTouchingLadder = mover.IsTouchingLadder();
            //Check if controller is climbing;
            bool _isClimbing = _isTouchingLadder && characterInput.GetVerticalMovementInput() != 0;

            //Climbing;
            if (currentControllerState == ControllerState.Climbing)
            {
                //Holding still on ladder
                if (!_isClimbing && _isTouchingLadder)
                {
                    return ControllerState.Climbing;
                }
                //Falling off ladder
                if (!_isClimbing && !_isTouchingLadder && !mover.IsGrounded())
                {
                    OnGroundContactLost();
                    return ControllerState.Falling;
                }
                //Getting off ladder
                if (!_isClimbing && mover.IsGrounded())
                {
            ...
            if (currentControllerState == ControllerState.Grounded)
            {
                // Debug.Log("Is grounded and climbing: " + _isClimbing);
                if (_isClimbing)
                {
                    OnGroundContactLost();
                    return ControllerState.Climbing;
